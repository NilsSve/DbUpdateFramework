...
PRINT 'Backing up computed and regular columns...';
...
-- Drop all schema-bound objects that depend on collation
PRINT 'Dropping schema-bound objects that depend on collation...';
DECLARE @DropSchemaBoundObjects NVARCHAR(MAX) = '';
SELECT @DropSchemaBoundObjects += 'DROP VIEW ' + QUOTENAME(s.name) + '.' + QUOTENAME(o.name) + ';' + CHAR(13)
FROM sys.objects o
JOIN sys.schemas s ON o.schema_id = s.schema_id
WHERE o.type IN ('V')  -- Only views
AND o.is_ms_shipped = 0;  -- Exclude system views

IF @DropSchemaBoundObjects <> ''
BEGIN
    PRINT 'Executing drop commands for schema-bound objects:';
    PRINT @DropSchemaBoundObjects;
    EXEC sp_executesql @DropSchemaBoundObjects;  -- Execute the drop commands
END
...
-- Step 1: Check for duplicates in ComputedColumnsBackup
PRINT 'Checking for duplicate entries in ComputedColumnsBackup...';
DECLARE @RecreateColumns NVARCHAR(MAX) = '';

SELECT TableName, ColumnName, COUNT(*) as DuplicateCount
FROM ##ComputedColumnsBackup
GROUP BY TableName, ColumnName
HAVING COUNT(*) > 1;
GO

-- If no duplicates found, proceed with recreation
PRINT 'Recreating columns...';
DECLARE @RecreateColumns NVARCHAR(MAX) = '';

-- Recreate all columns in the original order, but only take the first occurrence of each column
SELECT @RecreateColumns += 
    CASE 
        WHEN IsComputed = 1 THEN 'ALTER TABLE ' + TableName + ' ADD ' + ColumnName + ' AS (' + Definition + ');'
        ELSE 'ALTER TABLE ' + TableName + ' ADD ' + ColumnName + ' ' + Definition + ';'
    END + CHAR(13)
FROM (
    SELECT DISTINCT TableName, ColumnName, IsComputed, Definition,
           ROW_NUMBER() OVER (PARTITION BY TableName, ColumnName ORDER BY OrdinalPosition) as rn
    FROM ##ComputedColumnsBackup
) t
WHERE rn = 1
ORDER BY TableName, OrdinalPosition;

-- Debugging output to verify column processing
PRINT 'Columns to be recreated:';
...
-- Step 10: Recreate Columns
PRINT 'Recreating columns...';
DECLARE @RecreateColumns NVARCHAR(MAX) = '';

-- Recreate all columns in the original order
SELECT @RecreateColumns += 
    CASE 
        WHEN IsComputed = 1 THEN 'ALTER TABLE ' + TableName + ' ADD ' + ColumnName + ' AS (' + Definition + ');'
        ELSE 'ALTER TABLE ' + TableName + ' ADD ' + ColumnName + ' ' + Definition + ';'
...
SELECT @RecreateIndexes += 
    'IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ''' + PARSENAME(TableName, 1) + ''' AND COLUMN_NAME = ''' + PARSENAME(IndexDefinition, 1) + ''') ' +
    'BEGIN ' + IndexDefinition + '; END' + CHAR(13)
FROM ##IndexesBackup
